use ratatui::{
    ...
    text::{Span, Line, Text},
    widgets::{Block, Borders, BorderType, List, ListItem, Paragraph, Gauge, Wrap, Tabs},
    Frame,
};
use crate::ui::app::App;
use crate::core::heuristics::{FitStatus, Estimation};
use crate::core::system::GpuBackend;
...
fn render_header(f: &mut Frame, app: &App, area: Rect) {
    let sys = &app.system;
    let gpu_text = format!("{} ({:.1} GB VRAM)", sys.gpu.name, sys.gpu.vram_total_bytes as f64 / 1e9);
    let ram_text = format!("RAM: {:.1}/{:.1} GB", sys.ram_available_bytes as f64 / 1e9, sys.ram_total_bytes as f64 / 1e9);

    let info = format!(" {} | {} | CPU: {} ({} cores) ",
        "VRAMancer", gpu_text, sys.cpu_name, sys.cpu_cores);
...
fn render_models_list(f: &mut Frame, app: &mut App, area: Rect) {
    let items: Vec<ListItem> = app.filtered_models.iter().enumerate().map(|(i, m)| {
    ...
    // Simple scrolling:
    let start = if app.selected_index >= 10 {
        app.selected_index - 10
    } else {
        0
    };
    // This is naive. Better to use `ListState` in App, but user asked for simple struct.
    // I'll stick to naive rendering for now or let List handle it if I pass state.
    // Ratatui List doesn't auto scroll without State.
    // I will adjust `items` slice in a real implementation, but here I'll just pass all items
    // and let the user scroll via selection index? No, if selected is out of view, it's bad.

    // Let's use `ListState` properly in App if I could change App.
    // But I can also just create a ephemeral state here.
...
fn render_prediction(f: &mut Frame, app: &App, area: Rect) {
    let block = Block::default().borders(Borders::ALL).title(" Prediction ").border_type(BorderType::Rounded);
    ...
    if let Some(est) = &app.current_estimation {
        ...
        let vram_label = format!("{:.1} / {:.1} GB", est.vram_usage_bytes as f64 / 1e9, app.system.gpu.vram_total_bytes as f64 / 1e9);
        let vram_gauge = Gauge::default()
            .block(Block::default().title("VRAM Usage"))
            .gauge_style(Style::default().fg(if vram_ratio > 0.9 { Color::Red } else { Color::Cyan }))
        ...
        if est.ram_usage_bytes > 0 {
             ...
             let ram_label = format!("{:.1} / {:.1} GB (Offload)", est.ram_usage_bytes as f64 / 1e9, app.system.ram_available_bytes as f64 / 1e9);
        ...
        let metrics_text = vec![
            Line::from(vec![
                Span::raw("Est. Speed: "),
                Span::styled(format!("{:.1} tok/s", est.tokens_per_sec), Style::default().fg(Color::Green).add_modifier(Modifier::BOLD)),
            ]),
            Line::from(vec![
                Span::raw("Est. TTFT:  "),
                Span::styled(format!("{:.0} ms", est.ttft_ms), Style::default().fg(Color::Yellow)),
            ]),
            Line::from(""),
            Line::from(vec![Span::styled("Recommendation:", Style::default().add_modifier(Modifier::UNDERLINED))]),
...
fn render_settings(f: &mut Frame, app: &App, area: Rect) {
    let block = Block::default().borders(Borders::ALL).title(" Scenario ").border_type(BorderType::Rounded);
    ...
    let text = vec![
        ...
        Line::from(""),
        Line::from(Span::styled("Assumptions:", Style::default().add_modifier(Modifier::UNDERLINED))),
    ];
    ...
    let p = Paragraph::new(assumptions)
        .wrap(Wrap { trim: true });
